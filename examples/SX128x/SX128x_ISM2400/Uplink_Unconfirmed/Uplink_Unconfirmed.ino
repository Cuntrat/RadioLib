/*
  Uplink Unconfirmed - LoRaWAN ISM2400 Example

  Demonstrates UNCONFIRMED uplink transmission (server does NOT send ACK).
  This is the standard "fire-and-forget" mode.
  - Sends counter every second
  - Prints send result to serial
  - No downlink expected
*/
...
void setup() {
  ...
  Serial.println(F("Mode: Send counter every second (no ACK expected)\n"));
...
void loop() {
  ...
  // UNCONFIRMED uplink: sendReceive with fPort and no downlink expectation
  uint8_t down[256];
  size_t downLen = 0;
  ...
  if(st == RADIOLIB_ERR_NETWORK_NOT_JOINED) {
    ...
    counter++;
    if(downLen > 0) {
      Serial.print(F("  Unexpected downlink received: "));
