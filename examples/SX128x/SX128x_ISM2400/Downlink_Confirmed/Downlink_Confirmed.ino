/*
  Downlink Confirmed - LoRaWAN ISM2400 Example

  Demonstrates receiving CONFIRMED downlink messages.
  - Sends confirmed uplink to trigger RX windows
  - Server can send confirmed downlink (requires ACK)
  - Device automatically sends ACK in next uplink
  - Prints received downlink payload to serial
*/
...
void setup() {
  ...
  Serial.println(F("Mode: Listen for confirmed downlink (ACK required)\n"));
...
void loop() {
  ...
  uint8_t up[1] = { (uint8_t)(counter % 256) };

  // Send CONFIRMED uplink to trigger RX windows
  // This ensures we can receive confirmed downlink
  uint8_t down[256];
  size_t downLen = 0;
  ...
  // Send confirmed uplink (true parameter)
  ...
  if(st == RADIOLIB_ERR_NETWORK_NOT_JOINED) {
    ...
    if(downLen > 0) {
      // Check if this is a confirmed downlink (bit 7 of FHDR set)
      // RadioLib handles ACK automatically for confirmed frames
      
      Serial.println(F(""));
      Serial.println(F("  ╔════════════════════════════════════════╗"));
      Serial.println(F("  ║ ✓ DOWNLINK RECEIVED (CONFIRMED)       ║"));
      Serial.println(F("  ║   [ACK will be sent automatically]    ║"));
      Serial.println(F("  ╚════════════════════════════════════════╝"));
