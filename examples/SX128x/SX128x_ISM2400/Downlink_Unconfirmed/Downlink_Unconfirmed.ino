/*
  Downlink Unconfirmed - LoRaWAN ISM2400 Example

  Demonstrates receiving UNCONFIRMED downlink messages.
  - Sends empty uplink to trigger RX windows
  - Server can send unconfirmed downlink (no ACK required)
  - Device prints received downlink payload to serial
  - Uses receiveClassA for RX window management
*/
...
void setup() {
  ...
  Serial.println(F("Mode: Listen for unconfirmed downlink (no ACK required)\n"));
...
void loop() {
  ...
  // Send empty/minimal uplink to trigger RX windows
  uint8_t down[256];
  size_t downLen = 0;

  Serial.print(F("Sending uplink [counter="));
  ...
  if(st == RADIOLIB_ERR_NETWORK_NOT_JOINED) {
    ...
    if(downLen > 0) {
      Serial.println(F(""));
      Serial.println(F("  ╔════════════════════════════════════════╗"));
      Serial.println(F("  ║ ✓ DOWNLINK RECEIVED (UNCONFIRMED)     ║"));
      Serial.println(F("  ╚════════════════════════════════════════╝"));
